# waardepapieren-service chart

The Helm chart installs waardepapieren-service and by default the following dependencies using subcharts:
...
```bash
helm repo add waardepapieren-service https://raw.githubusercontent.com/ConductionNL/waardepapieren-service/master/api/helm/
helm repo update
```
...
```bash
helm install my-waardepapieren-service ConductionNL/waardepapieren-service --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `waardepapieren-service` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `waardepapieren-service` |
